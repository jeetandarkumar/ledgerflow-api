...
public class ApplicationDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
{
    ...
    {
        // When called via --startup-project src/CleanArch.API, the CWD is the API project
        // When called without --startup-project, CWD is the Infrastructure project
        // Try to find appsettings in multiple locations
        var basePaths = new[]
        {
            Directory.GetCurrentDirectory(),
            Path.Combine(Directory.GetCurrentDirectory(), "src", "CleanArch.API"),
            Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "CleanArch.API")),
        };
        ...
        var connectionString =
            ...
            ?? "Server=DESKTOP-72ACC7O;Database=CleanArchDb;Trusted_Connection=True;TrustServerCertificate=True;";
